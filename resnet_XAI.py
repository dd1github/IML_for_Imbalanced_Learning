...
__all__ = ['ResNet_s', 'resnet20', 'resnet32', 'resnet44', 'resnet56',
...
class ResNet_s(nn.Module):
    ...
    def forward(self, x):
        ...
        out_full_feat = self.layer3(out)
        
        out = F.avg_pool2d(out_full_feat, out_full_feat.size()[3])
        
        feats = out.view(out.size(0), -1)
        
        out = self.linear(feats)
        
        return out, out_full_feat, feats
...
def test(net):
    ...
    print("Total layers", 
          len(list(filter(lambda p: p.requires_grad and len(p.data.size())>1,
                          net.parameters()))))
